...
import { findNextDayWithLesson, findNextLessonDate, findNotifiedStudents, checkIsToday, dayInMilliseconds, } from './utils/functions';
...
export class DataBase {
    ...
    async getAllContributors(schoolName) {
        ...
        return Promise.all(classes
            ...
            .map((studentId) => this.getStudentBy_Id(studentId))).then((students) => students.filter((student) => student !== null).filter(({ role }) => [Roles.contributor, Roles.admin].includes(role)));
    ...
    async getHomeworkByDate({ classNameOrInstance, schoolName }, date) {
        ...
        if (Class) {
            ...
            return homework?.filter((hw) => Math.abs(hw.to.getTime() - date.getTime()) <= dayInMilliseconds &&
    ...
    async parseHomeworkToNotifications(currentDateForTest) {
        ...
        for (const cl of classes) {
            ...
            if (populatedClass.homework.length && populatedClass.students.length) {
                ...
                const notifiedStudentIds = findNotifiedStudents(populatedClass.students, currentDateForTest || new Date(), 24 * 60 * 60 * 1000).map(({ vkId }) => vkId);
    ...
    async addStudentToClass(vkIdOrStudentInstance, classNameOrInstance, schoolName) {
        ...
        if (Class && Student) {
            ...
            await Student.updateOne({
                class: Class._id,
                role: Class.students.length === 0 && Student.role === Roles.student
                    ? Roles.contributor
                    : Student.role,
            });
            return true;
    ...
    async removeStudentFromClass(vkIdOrStudentInstance) {
        ...
        if (PopulatedStudent) {
            const Class = await this.populate(PopulatedStudent.class);
            if (!Class)
                return true;
            await Class.updateOne({

...
export const findNextDayWithLesson = (schedule, lesson, currentWeekDay) => {
    let lastIndex = -1;
...
export const findNextLessonDate = (nextLessonWeekDay, { currentDate = new Date() } = {}) => {
...
export const toObject = (Document) => JSON.parse(JSON.stringify(Document));
export const isObjectId = (id) => {
    return mongoose.Types.ObjectId.isValid(id);
};
export const findNotifiedStudents = (students, notificationDate, maxRemindFrequency) => {
...
export const lessonsIndexesToLessonsNames = (lessonList, indexes) => {
...
export const filterContentByDate = (content, date) => {
    return content.filter((cont) => Math.abs(cont.to.getTime() - date.getTime()) <= dayInMilliseconds &&
        cont.to.getDate() === date.getDate());
};
export const mapHomeworkByLesson = (homework) => {

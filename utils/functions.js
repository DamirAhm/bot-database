...
const findNextLessonDate = (
  nextLessonWeekDay,
  { currentDate = new Date(), monthWith31 = [0, 2, 4, 6, 7, 9, 11] } = {}
) => {
  if (nextLessonWeekDay <= 7) {
    ...
    const maxDate = monthWith31.includes(currentDate.getMonth())
      ? 31
      : currentDate.getMonth() !== 1
      ? 30
      : currentDate.getFullYear() % 4 === 0
      ? 29
      : 28; //Возвращает количество дней в текущем месяце
  ...
  return students.filter(({ settings: sets, lastHomeworkCheck }) => {
    if (sets.notificationsEnabled) {
      ...
      if (
        ...
        Math.abs(
          notificationDate.getMinutes() - +sets.notificationTime.match(/\d*$/)
        ) <= 1
      ) {
...
const lessonsIndexesToLessonsNames = (lessonList, indexes) => {
  if (
    ...
    if (
      ...
      indexes.length > 0 &&
      indexes.every(
        (lesson) => Array.isArray(lesson) && lesson.every(Number.isInteger)
...
const mapHomeworkByLesson = (homework) => {
  if (homework instanceof Array) {
    return homework.reduce(
      (acc, c) => (
        acc.has(c.lesson) ? acc.get(c.lesson).push(c) : acc.set(c.lesson, [c]),
        acc
      ),
      new Map()
    );

...
function isClassPopulated(
  Document: ClassDocument | PopulatedClass
): Document is PopulatedClass {
...
function isStudentPopulated(
  ...
  return (
    ...
    (Document.class === null || "name" in Document.class)
  );
}
...
export class DataBase {
  ...
  async getAllContributors(schoolName?: string) {
    ...
    return Promise.all(
      ...
      (
        students.filter((student) => student !== null) as StudentDocument[]
      ).filter(({ role }) => [Roles.contributor, Roles.admin].includes(role))
    );
  ...
  async getStudentsFromClass(
    ...
    if (
      ...
      isClassPopulated(classNameOrInstance)
    ) {
  ...
  async createStudent(
    ...
    if (class_id) {
      ...
      if (Class) {
        ...
        newStudent.role =
          Class.students.length === 0 ? Roles.contributor : newStudent.role;
      }
    }
  ...
  async updateHomework(
    ...
    if (Class) {
      ...
      return await this.getClassBy_Id(Class._id).then(
        (cl) => cl?.homework || null
      );
    } else {
      return [];
  ...
  async removeOldHomework(classData: IClassData, maxDate: Date = new Date()) {
    ...
    if (Class) {
      ...
      const actualHomework = homework.filter(
        ({ to }) =>
          getPureDate(maxDate).getTime() - getPureDate(to).getTime() <= 0
      );
    ...
    try {
      ...
      if (School) {
        if (inRange(dayIndex, 1, 7)) {
          ...
          throw new Error(
            "Day index must be in range 1 to 7, got: " + dayIndex
          );
        }
      } else {
        return null;
    ...
    try {
      ...
      if (School) {
        if (inRange(dayIndex, 1, 6)) {
          ...
          throw new Error(
            "Day index must be in range 0 to 5, got: " + dayIndex
          );
        }
      } else {
        return false;
  ...
  getLessonAtSpecificTime(callSchedule: lessonCall[], date: Date) {
    ...
    while (
      compareTimes(time, lessonEnds[index]) &&
      index < lessonEnds.length - 1
    )
      index++;
  ...
  getNextCallTime(callSchedule: lessonCall[], date: Date) {
    ...
    if (compareTimes(currentLesson.start, currentTime))
  ...
  async togglePinAnnouncement(
    ...
    try {
      ...
      if (Class) {
        const announcement = Class.announcements.find(
  ...
  async changeSettings(
    ...
    if (Student) {
      await Student.updateOne({
        settings: { ...Student.settings, ...diffObject },
      });
      return true;
    } else {
      return false;
  ...
  async changeClass(
    ...
    if (Student.class !== null) {
      const removed = this.removeStudentFromClass(Student);

      if (!removed) return false;
    }

    return this.addStudentToClass(Student, newClass);
  ...
  async getClassByClassData({ classNameOrInstance, schoolName }: IClassData) {
    ...
    if (typeof classNameOrInstance === "string") {
      if (typeof schoolName === "string") {
        ...
        throw new Error(
          "School name must be a string if class name is provided"
        );
      }
    } else {
      Class = classNameOrInstance;

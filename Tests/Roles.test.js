...
describe("generateNewRoleUpCode", () => {
    ...
    it("should return null if can`t find class by name", async () => {
        const code = await DataBase.generateNewRoleUpCode(
            "not real class name"
        );
...
describe("removeRoleUpCode", () => {
    ...
    it("should return false if con`t find class by name", async () => {
        const isDeleted = await DataBase.removeRoleUpCode(
            "not a name",
            uuid4()
        );
    ...
    it("should throw type error if code is not valid uuid4 code", async () => {
        return DataBase.removeRoleUpCode(
            MockClass.name,
            "not valid code"
    ...
    it("should remove only one code", async () => {
        const code = await DataBase.generateNewRoleUpCode(MockClass.name);
        const initialLength = (await DataBase.getClassBy_Id(MockClass._id))
            .roleUpCodes.length;

        await DataBase.removeRoleUpCode(MockClass.name, code);

        const newLength = (await DataBase.getClassBy_Id(MockClass._id))
            .roleUpCodes.length;
...
describe("checkCodeValidity", () => {
    ...
    it("should return false if code is not valid uuid4 code", async () => {
        const response = await DataBase.checkCodeValidity(
            MockClass,
            "not valid code"
        );
    ...
    it("should throw error if passed Document but not a class", async () => {
        ...
        const f = async () =>
            await DataBase.checkCodeValidity(MockStudent, code);

        return expect(f).toThrowError(
            TypeError("className must be a string or Document")
        );
    });
    ...
    it("shouldn`t add codes to class", async () => {
        ...
        return expect(
            (await DataBase.getClassBy_Id(MockClass._id)).roleUpCodes.length
        ).toBe(1);
    });
...
describe("activateRoleUpCode", () => {
    ...
    it("should throw error if student don`t have class property", async () => {
        ...
        return DataBase.activateCode(MockStudent.vkId, code).catch((err) =>
...
describe("backStudentToInitialRole", () => {
    ...
    it("should return true if all is ok", async () => {
        const result = await DataBase.backStudentToInitialRole(
            MockStudent.vkId
        );
    ...
    it("should throw error if vkId is not valid (number)", () => {
        return DataBase.backStudentToInitialRole("not a number").catch((e) =>

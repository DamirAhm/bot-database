...
const createMockStudent = ({
    ...
    nt: notificationTime = "00:00",
    lhc: lastHomeworkCheck = new Date(0),
} = {}) => {
    return {
        settings: {
...
describe("findNextLessonDate", () => {
    it("should return date of next this week day", () => {
        expect(
            findNextLessonDate(1, { currentDate: new Date(2020, 0, 1) })
        ).toEqual(new Date(2020, 0, 6));
        expect(
            findNextLessonDate(2, { currentDate: new Date(2020, 1, 28) })
        ).toEqual(new Date(2020, 2, 3));
        expect(
            findNextLessonDate(1, { currentDate: new Date(2020, 2, 13) })
        ).toEqual(new Date(2020, 2, 16));
        expect(
            findNextLessonDate(1, { currentDate: new Date(2020, 3, 30) })
        ).toEqual(new Date(2020, 4, 4));
        expect(
            findNextLessonDate(1, { currentDate: new Date(2020, 4, 30) })
        ).toEqual(new Date(2020, 5, 1));
        expect(
            findNextLessonDate(1, { currentDate: new Date(2020, 4, 31) })
        ).toEqual(new Date(2020, 5, 1));
        expect(
            findNextLessonDate(6, { currentDate: new Date(2020, 2, 13) })
        ).toEqual(new Date(2020, 2, 14));
        expect(
            findNextLessonDate(7, { currentDate: new Date(2020, 2, 13) })
        ).toEqual(new Date(2020, 2, 15));
        expect(
            findNextLessonDate(1, { currentDate: new Date(2020, 2, 15) })
        ).toEqual(new Date(2020, 2, 16));
    });
});
...
describe("findNotifiedStudents", () => {
    it("should filter students that shouldn't get notification", () => {
        ...
        const studentNP2 = createMockStudent({
            lhc: new Date(2020, 0, 1, 0, 0, 0),
        });
        const maxRemindFreq = config["REMIND_AFTER"];

        const notificationsArray = findNotifiedStudents(
            [studentP, studentP1, studentP2, studentNP, studentNP1, studentNP2],
            notificationDate,
            maxRemindFreq
        );
        ...
        expect(
            notificationsArray.includes(studentP) &&
                notificationsArray.includes(studentP1) &&
                notificationsArray.includes(studentP2)
        ).toBe(true);
    });
});
...
describe("lessonsIndexesToLessonsNames", () => {
    ...
    it("should throw error if some of indexes is bigger than lessons length", () => {
        ...
        expect(() =>
            lessonsIndexesToLessonsNames(lessons, indexes)
        ).toThrowError(ReferenceError);
    });
    it("should throw error if some of lessons is not type of string / it's length is 0 / it's not an array ", () => {
        expect(() => lessonsIndexesToLessonsNames(["1", 2], [])).toThrowError(
            new TypeError("LessonList must be array of strings")
        ); //not type of string
        expect(() => lessonsIndexesToLessonsNames([], [])).toThrowError(
            new TypeError("LessonList must be array of strings")
        ); //length is 0
        expect(() =>
            lessonsIndexesToLessonsNames("not an array", [])
        ).toThrowError(new TypeError("LessonList must be array of strings")); //not an array
    });
    it("should throw error if indexes is not array / indexes length is 0  / some of array elements isn't array of integers", () => {
        expect(() =>
            lessonsIndexesToLessonsNames(["1", "2"], "not an array")
        ).toThrowError(
            new TypeError(
        ...
        expect(() =>
            lessonsIndexesToLessonsNames(["1", "2"], [["1", 2]])
        ).toThrowError(
            new TypeError(
                "lessonsIndexesByDays must be array of arrays of integers"
            )
        ); //some of elements isn't numbers
        expect(() =>
            lessonsIndexesToLessonsNames(["1", "2"], [[1.4, 2]])
        ).toThrowError(
            new TypeError(
                "lessonsIndexesByDays must be array of arrays of integers"
            )
        ); //some of elements isn't integers
    });
});
